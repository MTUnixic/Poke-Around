...
class AudioBufferData
{
    ...
    public function new(numChannels:Int, numSamples:Int, sampleRate:Float) {
        this.channels = backend.util.grig.audio.python.numpy.Numpy.zeros(python.Tuple2.make(numChannels, numSamples));
    ...
    public function resample(ratio:Float, repitch:Bool = false):AudioBuffer {
        ...
        var inputIndices = backend.util.grig.audio.python.numpy.Numpy.zeros(python.Tuple2.make(1, newNumSamples));
        ...
        for (c in 0...numChannels) {
            var channel = get(c);
            var indices:Ndarray = python.Syntax.code('{0}.arange(0, {1}.shape[0])', backend.util.grig.audio.python.numpy.Numpy, channel);
            var int = backend.util.grig.audio.python.scipy.Interpolate.interp1d(indices, cast channel, 'linear');
