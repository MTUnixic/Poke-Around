package backend.util.grig.audio.python; #if python

import backend.util.grig.audio.AudioBuffer;
import backend.util.grig.audio.AudioCallback;
import backend.util.grig.audio.AudioChannel;
import backend.util.grig.audio.NumericTypes;
import backend.util.grig.audio.python.numpy.Ndarray;
import python.Dict;
...
using backend.util.grig.audio.AudioBufferTools;
...
class AudioInterface
{
    ...
    private var api:backend.util.grig.audio.Api;
    ...
    private function callbackHandler(input:python.Bytes, frameCount:Int, timeInfo:Dict<String, Float>, statusFlags:Int):Tuple2<python.Bytearray, Int>
    {
        if (input != null) {
            ...
            kwargs.set('dtype', backend.util.grig.audio.python.numpy.Numpy.float32);
            var inputDataInterleaved = backend.util.grig.audio.python.numpy.Numpy.frombuffer(input, kwargs);
            var inputData:Ndarray = python.Syntax.code('[{0}[idx::{1}] for idx in range({1})]', inputDataInterleaved, outputNumChannels);
            // var inputData:Ndarray = backend.util.grig.audio.python.numpy.Numpy.zeros(python.Tuple2.make(outputNumChannels, Std.int(frameCount)));
        ...
        var streamInfo = new backend.util.grig.audio.AudioStreamInfo();
        ...
        var output = backend.util.grig.audio.python.numpy.Numpy.dstack(outputArrayArgs).flatten().astype('float32').tobytes();
        return new Tuple2([python.Syntax.code('bytes({0})', output), 0]);
    }

    public function new(_api:backend.util.grig.audio.Api = backend.util.grig.audio.Api.Unspecified)
    {
        api = _api;
        pyAudio = new PyAudio();
    }

    public static function getApis():Array<backend.util.grig.audio.Api>
    {
        var pyAudio = new PyAudio();
        var apiCount = pyAudio.get_host_api_count();
        var apis = new Array<backend.util.grig.audio.Api>();
        for (i in 0...apiCount) {
            var apiInfo = pyAudio.get_host_api_info_by_index(i);
            var api = backend.util.grig.audio.PortAudioHelper.apiFromName(apiInfo.get('name'));
            apis.push(api);
        }
        return apis;
    }
    ...
    private function addSampleRatesToPortInfo(portInfo:PortInfo):Void
    {
        ...
        for (sampleRate in backend.util.grig.audio.SampleRate.commonSampleRates) {
            try {
    ...
    public function getPorts():Array<PortInfo>
    {
        ...
        var apiCount = pyAudio.get_host_api_count();
        var apiName = backend.util.grig.audio.PortAudioHelper.nameFromApi(api);
        for (i in 0...apiCount) {
            var apiInfo = pyAudio.get_host_api_info_by_index(i);
            if (apiInfo.get('name') == apiName || api == backend.util.grig.audio.Api.Unspecified) {
